...
const redirectStatusStorage = useSessionStorage('VUEPRESS_REDIRECT_LOCALES', {});
export default defineComponent({
    ...
    setup() {
        ...
        const redirect = () => {
            router.replace(routePath.value.replace(routeLocale.value, info.value.localePath));
        };
        watch(routePath, () => {
        ...
        return () => h(TransitionGroup, { name: 'redirect-modal-fade' }, () => showModal.value
            ? h('div', { key: 'mask', class: 'redirect-modal-mask' }, h('div', {
                key: 'popup',
                class: 'redirect-modal-wrapper',
            }, [
                h('div', { class: 'redirect-modal-content' }, locale.value?.hint.map((text) => h('p', text))),
                h('button', {
                    type: 'button',
