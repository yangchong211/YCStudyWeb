...
- 有状态组件：StatefulWidget
    - 有状态组件是界面会发生变化的组件，比如：Scrollable，Animatable
    ```
    ...
    class MyTest4 extends State<TestMain4> {
      @override
      Widget build(BuildContext context) {
        return null;
      }
...
- 无状态组件：StatelessWidget
    - 无状态组件指的是界面不发生变化的组件，比如Text，AssetImage等。
    ```
    class TestMain5 extends StatelessWidget{
      @override
      Widget build(BuildContext context) {
    ...
    class MyTest extends State<TestWidget> {
      @override
      Widget build(BuildContext context) {
        return null;
      }
